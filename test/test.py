...
async def test_project(dut):
    ...
    # --- Capture and Real Checker Loop ---
    # We will test the first 10 samples generated by the biquad engine
    for count in range(500):
        ...
        internal_s_val = dut.uio_out.value.signed_integer

        dut._log.info(f"Sample {count+1}: UART Received = {signed_uart_val} | Internal S = {internal_s_val}")

        # 4. ASSERTION CHECKER
        # This acts as the mathematical check to ensure no bits were warped during shifting or streaming
        assert signed_uart_val == internal_s_val, (
            f"Mismatch detected at sample {count+1}! "
            f"UART sent {signed_uart_val}, but internal register holds {internal_s_val}"
        )

    dut._log.info("SUCCESS: All serial samples matched the internal sinusoid values perfectly!")
